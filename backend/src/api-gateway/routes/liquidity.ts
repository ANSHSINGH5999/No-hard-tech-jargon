...
import { rawAmountToDisplay } from "../../dex-integration/index.js";
...
export const liquidityRoutes: FastifyPluginAsync<{
  prisma: PrismaClient;
  ...
  fastify.get("/liquidity/pool-stats", async () => {
    try {
      const [snapshot, oracle, programs] = await Promise.all([
        dexIntegrationService.getCurrentPoolSnapshot(),
        dexIntegrationService.getOracleSnapshot(900),
        dexIntegrationService.getLiquidityMiningPrograms(),
      ]);
      const reserveXlm = rawAmountToDisplay(snapshot.reserveXlmRaw);
      const reserveSxlm = rawAmountToDisplay(snapshot.reserveSxlmRaw);

      return {
        reserveXlm,
        reserveSxlm,
        totalLpSupply: rawAmountToDisplay(snapshot.totalLpSupplyRaw),
        price: oracle.spotPrice,
        feeBps: snapshot.feeBps,
        tvl: reserveXlm + reserveSxlm * oracle.twapPrice,
        oracle,
        activeLiquidityMiningPrograms: programs.filter((program) => program.status === "active")
          .length,
      };
    } catch {
      return {
        ...
        tvl: 0,
        oracle: null,
        activeLiquidityMiningPrograms: 0,
      };
    }
  });

  fastify.get("/liquidity/mining/programs", async () => {
    const programs = await dexIntegrationService.getLiquidityMiningPrograms();
    return {
      programs: programs.map((program) => ({
        ...program,
        rewardPerDay: rawAmountToDisplay(program.rewardPerDayRaw),
        rewardPerDayRaw: program.rewardPerDayRaw.toString(),
        minLpTokens: rawAmountToDisplay(program.minLpTokensRaw),
        minLpTokensRaw: program.minLpTokensRaw.toString(),
        totalRewardsRaw: program.totalRewardsRaw?.toString() ?? null,
        distributedRewardsRaw: program.distributedRewardsRaw?.toString() ?? "0",
      })),
      total: programs.length,
    };
  });

  fastify.get("/liquidity/mining/position/:wallet", async (request) => {
    const { wallet } = request.params as { wallet: string };
    const position = await dexIntegrationService.getLiquidityMiningPosition(wallet);
    return {
      wallet: position.wallet,
      lpTokens: position.lpTokens,
      lpTokensRaw: position.lpTokensRaw,
      eligiblePrograms: position.eligiblePrograms.map((program) => ({
        ...program,
        estimatedDailyRewardsRaw: program.estimatedDailyRewardsRaw.toString(),
      })),
    };
  });
};
