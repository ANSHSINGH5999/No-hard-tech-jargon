...
export function registerErrorHandling(fastify: FastifyInstance): void {
  ...
  fastify.setErrorHandler((error, request, reply) => {
    ...
    if (normalizedError.validation || normalizedError.cause instanceof ZodError) {
      const zodErr = normalizedError.cause as ZodError | undefined;
      const response: ApiErrorResponse = {
        error: "Validation failed",
        code: "VALIDATION_ERROR",
        statusCode: 400,
    ...
    // Known error codes
    const mapped = ERROR_CODE_MAP[normalizedError.message];
    if (mapped) {
      const response: ApiErrorResponse = {
