...
pub enum DataKey {
    ...
    ReferenceSupply,
    // LP Pool contract for automatic mining program updates
    LpPoolContract,
}
...
impl GovernanceContract {
    ...
    /// Set LP Pool contract address for automatic mining program updates. Admin-only.
    pub fn set_lp_pool(env: Env, lp_pool: Address) {
        ...
        env.storage()
            .instance()
            .set(&DataKey::LpPoolContract, &lp_pool);
    }

    /// Get LP Pool contract address.
    pub fn get_lp_pool(env: Env) -> Option<Address> {
        extend_instance(&env);
        env.storage().instance().get(&DataKey::LpPoolContract)
    }
